  ...
  try {
    ...
    if (!isNaN(id)) {
      const result = await pool.query(
        ...
        "SELECT a.title, a.id, a.description, a.image_url, a.minslots, a.maxslots, a.price, a.startdate, SUM(b.quantity) AS total_quantity FROM bookings AS b right JOIN activities AS a ON b.activities_id = a.id WHERE a.id = $1 AND a.startdate > NOW() AND a.active = true GROUP BY a.maxslots, a.title, a.id, a.description, a.image_url, a.minslots, a.price, a.startdate ORDER BY a.startdate ASC;", [id] );
      return res.status(200).json(result.rows[0]);
    } else res.status(500).json({ message: "Activity not found", error:error.message });
  } catch (error) {
    res.status(500).json({ message: "Activity not found", error:error.message });
  }
});
